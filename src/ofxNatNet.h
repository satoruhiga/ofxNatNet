...
public:
	...
	inline const Marker& getFilterdMarker(size_t index) { return filterd_markers[index]; }
	
	inline const size_t getNumRigidBody() { return rigidbodies.size(); }
	inline const RigidBody& getRigidBodyAt(int index) { return *rigidbodies_arr[index]; }
	
	inline const bool hasRigidBody(int id) { return rigidbodies.find(id) != rigidbodies.end(); }
